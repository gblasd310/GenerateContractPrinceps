...
def GET_DATA_CONTRACTS(DIR_DATA_CSV, DIR_OUTPUT_FILES):
    data = pd.read_csv(DIR_DATA_CSV, encoding='utf-8')
...
def main():
    ...
    DIR_OUTPUT_FILES = ''
     ...
     # create the threads
    threads = [Thread(target=GET_CONTRACT_PV, args=(DIR_INPUTS_FILES[i], DIR_OUTPUTS_FILES[i])) for i in range(len(DIR_INPUTS_FILES))]
